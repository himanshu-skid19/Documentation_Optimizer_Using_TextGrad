...
def download_twitter_api_spec():
    """Download Twitter API OpenAPI specification."""
    print("Downloading Twitter API specification...")
    
    twitter_url = "https://api.twitter.com/2/openapi.json"
    try:
        response = requests.get(twitter_url)
        if response.status_code == 200:
            spec_path = "twitter_api.json"
            with open(spec_path, "w", encoding="utf-8") as f:
                f.write(response.text)
            print(f"Twitter API specification downloaded to {spec_path}")
            return spec_path
        else:
            print(f"Failed to download Twitter API spec: {response.status_code}")
    except Exception as e:
        print(f"Error downloading Twitter API spec: {e}")
    
    # Fallback to a public mirror if direct download fails
    fallback_url = "https://raw.githubusercontent.com/twitterdev/twitter-api-openapi/main/dist/twitter-api-v2-openapi-3.0.json"
    try:
        ...
        if response.status_code == 200:
            ...
            with open(spec_path, "w", encoding="utf-8") as f:
                f.write(response.text)
            print(f"Twitter API specification downloaded from mirror to {spec_path}")
            return spec_path
    except Exception as e:
        print(f"Error downloading Twitter API spec from mirror: {e}")
    
    return None

def download_spotify_api_spec():
    """Download Spotify API OpenAPI specification."""
    print("Downloading Spotify API specification...")
    
    # Spotify doesn't publish an official OpenAPI spec, but community versions exist
    spotify_url = "https://raw.githubusercontent.com/sonallux/spotify-web-api/main/fixed-spotify-open-api.json"
    
    response = requests.get(spotify_url)
    if response.status_code == 200:
        spec_path = "spotify_api.json"
        with open(spec_path, "w", encoding="utf-8") as f:
            f.write(response.text)
        print(f"Spotify API specification downloaded to {spec_path}")
        return spec_path
    else:
        print(f"Failed to download Spotify API spec: {response.status_code}")
        return None

def download_stripe_api_spec():
    """Download Stripe API OpenAPI specification."""
    print("Downloading Stripe API specification...")
    
    # Stripe provides OpenAPI specs
    stripe_url = "https://raw.githubusercontent.com/stripe/openapi/master/openapi/openapi_spec.json"
    
    response = requests.get(stripe_url)
    if response.status_code == 200:
        spec_path = "stripe_api.json"
        with open(spec_path, "w", encoding="utf-8") as f:
            f.write(response.text)
        print(f"Stripe API specification downloaded to {spec_path}")
        return spec_path
    else:
        print(f"Failed to download Stripe API spec: {response.status_code}")
        return None
...
def create_documentation_examples(endpoints):
    ...
    for endpoint in tqdm(endpoints, desc="Creating examples"):
        ...
        all_examples.append({
            "instruction": f'''Review and suggest improvements for this API documentation:
            {format_full_documentation(endpoint).split('## Example')[0]}  # Exclude examples to make it "incomplete"
            "response": f"""Here are suggestions to improve this API documentation:
...
def main():
    ...
    # 3. Add Swagger Petstore specification
    ...
    # 4. Add Twitter API specification
    twitter_spec = download_twitter_api_spec()
    if twitter_spec:
        twitter_endpoints = process_openapi_spec(twitter_spec)
        all_endpoints.extend(twitter_endpoints)
    
    # 5. Add Spotify API specification
    spotify_spec = download_spotify_api_spec()
    if spotify_spec:
        spotify_endpoints = process_openapi_spec(spotify_spec)
        all_endpoints.extend(spotify_endpoints)
    
    # 6. Add Stripe API specification
    stripe_spec = download_stripe_api_spec()
    if stripe_spec:
        stripe_endpoints = process_openapi_spec(stripe_spec)
        all_endpoints.extend(stripe_endpoints)
    ...
    print(f"Sources: GitHub API, Twitter API, Spotify API, Kaggle API, Swagger Petstore")
